...
import { houseOwnerService, HouseOwner } from '@/api/houseOwnerService';
import { toast } from 'sonner';
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogFooter } from '@/components/ui/dialog';

export default function HouseOwnersPage() {
    const { t } = useTranslation();
    const navigate = useNavigate();
    const location = useLocation();
    const [items, setItems] = useState<HouseOwner[]>([]);
    const [loading, setLoading] = useState(true);
    const [searchQuery, setSearchQuery] = useState('');
    const [isDeleteOpen, setIsDeleteOpen] = useState(false);
    const [selectedItem, setSelectedItem] = useState<HouseOwner | null>(null);
    ...
    const loadHouseOwners = async () => {
        try {
            const data = await houseOwnerService.getHouseOwners();
            setItems(data.items || []);
        } catch (err) {
            console.error('Failed to load house owners:', err);
            toast.error(t('houseowners.errors.loadFailed'));
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDelete = async () => {
        ...
        try {
            await houseOwnerService.deleteHouseOwner(selectedItem.id);
            setIsDeleteOpen(false);
            loadHouseOwners();
            toast.success(t('houseowners.deleteSuccess') || 'House owner deleted successfully');
        } catch (error) {
            console.error('Failed to delete house owner:', error);
            toast.error(t('houseowners.deleteError') || 'Failed to delete house owner');
        }
    };

    const openDeleteModal = (item: HouseOwner) => {
        ...
        return (
            <div className="flex items-center justify-center min-h-screen bg-background">
                <div className="text-center p-8">
                    ...
                    <h2 className="text-xl font-bold text-foreground">{t('houseowners.loading')}</h2>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="min-h-screen bg-background pb-20">
                ...
                <div className="px-6 py-4">
                    <div className="flex items-center justify-between mb-6">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h1 className="text-xl font-bold text-foreground">{t('houseowners.title')}</h1>
                                <p className="text-sm text-muted-foreground">{t('houseowners.subtitle')}</p>
                            </div>
                        </div>
                        <div className="flex items-center gap-3">
                            ...
                            <Button
                                onClick={() => navigate('/dashboard/houseowners/new', { state: { returnTo: location.pathname } })}
                                className="bg-primary hover:bg-primary/90 text-primary-foreground gap-2"
                            >
                                <Plus className="w-4 h-4" />
                                {t('houseowners.newHouseOwner')}
                            </Button>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row gap-4">
                        <div className="relative flex-1">
                            ...
                            <input
                                ...
                                placeholder={t('houseowners.searchPlaceholder')}
                                value={searchQuery}
            ...
            <div className="px-6 mt-8 max-w-7xl mx-auto">
                <div className="mb-6 flex justify-between items-center">
                    <p className="text-sm text-muted-foreground">
                        {t('houseowners.showing')} <span className="font-semibold text-foreground">{filteredItems.length}</span> {t('houseowners.houseOwnersCount')}
                    </p>
                </div>
                ...
                {filteredItems.length === 0 ? (
                    <div className="text-center py-16 bg-card rounded-xl border border-border border-dashed">
                        ...
                        <h3 className="text-lg font-medium text-foreground">{t('houseowners.noHouseOwners')}</h3>
                        <p className="mt-1 text-sm text-muted-foreground max-w-sm mx-auto">
                            {searchQuery ? t('houseowners.noHouseOwnersDescSearch') : t('houseowners.noHouseOwnersDescEmpty')}
                        </p>
                        {!searchQuery && (
                            <div className="mt-6">
                                <Button
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {filteredItems.map((item) => (
                            <div
                                key={item.id}
                                onClick={() => navigate(`/dashboard/houseowners/${item.id}/edit`, { state: { returnTo: location.pathname } })}
                            ...
                            >
                                ...
                                <div className="mt-4 pt-4 border-t border-border flex items-center justify-between">
                                    <div className={`flex items-center gap-1.5 text-xs font-medium px-2 py-1 rounded-full ${item.insured
                                        ...
                                        {item.insured ? (
                                            <>
                                                ...
                                                {t('houseowners.insured')}
                                            </>
                                        ) : (
                                            <>
                                                <ShieldAlert className="w-3 h-3" />
                                                {t('houseowners.uninsured')}
                                            </>
                                        )}
                                    </div>
                                    <Button
            ...
            <Dialog open={isDeleteOpen} onOpenChange={setIsDeleteOpen}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>{t('houseowners.deleteTitle') || 'Delete House Owner'}</DialogTitle>
                    </DialogHeader>
                    <div className="py-4">
                        <p>{t('houseowners.deleteConfirmation') || `Are you sure you want to delete ${selectedItem?.name}? This action cannot be undone.`}</p>
                    </div>
                    <DialogFooter>
