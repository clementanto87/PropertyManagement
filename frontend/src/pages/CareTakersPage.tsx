...
import { careTakerService, CareTaker } from '@/api/careTakerService';
...
export default function CareTakersPage() {
    ...
    const [items, setItems] = useState<CareTaker[]>([]);
    const [loading, setLoading] = useState(true);
    const [searchQuery, setSearchQuery] = useState('');
    const [isDeleteOpen, setIsDeleteOpen] = useState(false);
    const [selectedItem, setSelectedItem] = useState<CareTaker | null>(null);
    ...
    const loadCareTakers = async () => {
        try {
            ...
            console.error('Failed to load care takers:', err);
            toast.error(t('caretakers.errors.loadFailed'));
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDelete = async () => {
        ...
        try {
            await careTakerService.deleteCareTaker(selectedItem.id);
            setIsDeleteOpen(false);
            loadCareTakers();
            toast.success(t('caretakers.deleteSuccess') || 'Care taker deleted successfully');
        } catch (error) {
            console.error('Failed to delete care taker:', error);
            toast.error(t('caretakers.deleteError') || 'Failed to delete care taker');
        }
    };

    const openDeleteModal = (item: CareTaker) => {
        ...
        return (
            <div className="flex items-center justify-center min-h-screen bg-background">
                <div className="text-center p-8">
                    ...
                    <h2 className="text-xl font-bold text-foreground">{t('caretakers.loading')}</h2>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="min-h-screen bg-background pb-20">
                ...
                <div className="px-6 py-4">
                    <div className="flex items-center justify-between mb-6">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h1 className="text-xl font-bold text-foreground">{t('caretakers.title')}</h1>
                                <p className="text-sm text-muted-foreground">{t('caretakers.subtitle')}</p>
                            </div>
                        </div>
                        <div className="flex items-center gap-3">
                            ...
                            <Button
                                onClick={() => navigate('/dashboard/caretakers/new', { state: { returnTo: location.pathname } })}
                                className="bg-primary hover:bg-primary/90 text-primary-foreground gap-2"
                            >
                                <Plus className="w-4 h-4" />
                                {t('caretakers.newCareTaker')}
                            </Button>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row gap-4">
                        <div className="relative flex-1">
                            ...
                            <input
                                ...
                                placeholder={t('caretakers.searchPlaceholder')}
                                value={searchQuery}
            ...
            <div className="px-6 mt-8 max-w-7xl mx-auto">
                <div className="mb-6 flex justify-between items-center">
                    <p className="text-sm text-muted-foreground">
                        {t('caretakers.showing')} <span className="font-semibold text-foreground">{filteredItems.length}</span> {t('caretakers.careTakersCount')}
                    </p>
                </div>
                ...
                {filteredItems.length === 0 ? (
                    <div className="text-center py-16 bg-card rounded-xl border border-border border-dashed">
                        ...
                        <h3 className="text-lg font-medium text-foreground">{t('caretakers.noCareTakers')}</h3>
                        <p className="mt-1 text-sm text-muted-foreground max-w-sm mx-auto">
                            {searchQuery ? t('caretakers.noCareTakersDescSearch') : t('caretakers.noCareTakersDescEmpty')}
                        </p>
                        {!searchQuery && (
                            <div className="mt-6">
                                <Button
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {filteredItems.map((item) => (
                            <div
                                key={item.id}
                                onClick={() => navigate(`/dashboard/caretakers/${item.id}/edit`, { state: { returnTo: location.pathname } })}
                                    ...
                                    <div className={`flex items-center gap-1.5 text-xs font-medium px-2 py-1 rounded-full ${item.insured
                                        ...
                                        {item.insured ? (
                                            <>
                                                ...
                                                {t('caretakers.insured')}
                                            </>
                                        ) : (
                                            <>
                                                <ShieldAlert className="w-3 h-3" />
                                                {t('caretakers.uninsured')}
                                            </>
                                        )}
                                    </div>
                                    <Button
            ...
            <Dialog open={isDeleteOpen} onOpenChange={setIsDeleteOpen}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>{t('caretakers.deleteTitle') || 'Delete Care Taker'}</DialogTitle>
                    </DialogHeader>
                    <div className="py-4">
                        <p>{t('caretakers.deleteConfirmation') || `Are you sure you want to delete ${selectedItem?.name}? This action cannot be undone.`}</p>
                    </div>
                    <DialogFooter>
